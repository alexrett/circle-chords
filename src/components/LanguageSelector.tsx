...
export default function LanguageSelector({ value, onChange }: { value: string; onChange: (lang: string) => void }) {
  const [options, setOptions] = useState<string[]>(['ru', 'en'])

  useEffect(() => {
    setOptions(Object.keys((i18n as any).store.data || { en: {}, ru: {} }))
  }, [])

  return (
    <label className="inline-flex items-center gap-2 text-sm">
      <span>{(i18n.t('controls.language') as string) || 'Language'}</span>
      <select
